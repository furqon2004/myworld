...
export class HeartText {
  constructor(scene) {
    ...
    this.config = {
      text: "Only For U, Anggia zahrariayani",
      size: 20,
      height: 2,
      curveSegments: 12,
      bevelEnabled: true,
  ...
  renderText3D(newText) {
    ...
    lines.forEach((line, i) => {
      ...
      mesh.position.set(this.config.position.x - textWidth / 2, this.config.position.y - i * lineHeight, this.config.position.z);
    ...
    switch (this.effectType) {
      case "none":
        this.textMeshes.forEach((mesh, idx) => {
          ...
          mesh.position.set(this.config.position.x - textWidth / 2, this.config.position.y - idx * this.config.size * 1.2, this.config.position.z);
  ...
  _createTextMesh(line, lineIdx) {
    ...
    const mesh = new THREE.Mesh(textGeometry, textMaterial);
    mesh.position.set(this.config.position.x - textWidth / 2, this.config.position.y - lineIdx * this.config.size * 1.2, this.config.position.z);
    if (this.config.height < 1) {
      mesh.scale.z = this.config.height;
    }
    return mesh;
